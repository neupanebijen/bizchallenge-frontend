...
const Home = ({ setPageChangeColor }) => {
  ...
  const scrollOnGesture = (e) => {
    ...
    setTimeout(() => setTouchActive(true), 900)
    if (pageCoordinate - touchTrack < 0) {
      if (pageIndex === pageData.length - 2) {
        setIndexPositive(true)
        changePageData(0)
        return
      }
    ...
    setIndexPositive(true)
    changePageData(pageIndex === 0 ? 0 : pageIndex - 1)
  }
  ...
  const keyDown = (e) => {
    if (e.keyCode === 40) {
      if (pageIndex === pageData.length - 1) return
      changePageData(pageIndex + 1)
    }
    if (e.keyCode === 38) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {size.width > 900 && pageIndex !== pageData.length - 1 && (
            <Menu
              changePageData={changePageData}
              loadThirdPage={() => {}}
              hideMenu={pageIndex === 0 ? false : true}
          ...
          <AnimatePresence exitBeforeEnter>
            <HeroMainContainer key={data.key}>
              ...
              {size.width > 900 && pageIndex === pageData.length - 1 && (
                <FooterContainer>
                  <ReviewContainer>
                    <h1>Some Business Information</h1>
                  </ReviewContainer>
                  <Footer />
                </FooterContainer>
              )}
